...
if __name__ == "__main__":
    ...
    results_dir = os.path.join(cfg.project_ws, "by_station")
    os.makedirs(results_dir, exist_ok=True)

    station_metadata = os.path.join(cfg.data_dir, "station_metadata.csv")
    sites = get_flux_sites(station_metadata, crop_only=False, western_only=False, header=1)
    run_pest_sequence(cfg, results_dir, select_stations=sites, pdc_remove=True, overwrite=False)
