...
def compare_ssebop(fid: str, flux_file: str, model_output: str, plots: SamplePlots,
                   gap_tolerance: int = 5, ssebop_eto_source: str = "eto_corr"):
...
def _verbose_monthly_summary(site_id: str, monthly: dict) -> Tuple[Optional[str], Optional[str]]:
    """Print summary statistics for SWIM vs SSEBop comparison."""
    rmse_all = {
        k.split("_", 1)[1]: v
    ...
    if "rmse_swim" in monthly and "rmse_ssebop" in monthly:
        best_pair_model = "swim" if monthly["rmse_swim"] <= monthly["rmse_ssebop"] else "ssebop"
    print("Best swim vs ssebop:", best_pair_model if best_pair_model else "NA")
...
if __name__ == "__main__":
    ...
    for ee, site_id in enumerate(sites):
        ...
        monthly = compare_ssebop(site_id, flux_file, out_csv, plots, gap_tolerance=5)
