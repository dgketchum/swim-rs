"""
DisALEXI ET fraction zonal statistics export module.

Export per-scene DisALEXI ET fraction zonal means for polygons to Google Cloud Storage as CSVs.
"""
import os
...
def export_disalexi_zonal_stats(
    shapefile,
    bucket,
    feature_id='FID',
    select=None,
    ...
    """
    Export per-scene DisALEXI ET fraction zonal means for polygons to GCS CSVs.

    DisALEXI computes ET directly, so ET fraction is calculated as ET / ET_reference.
    ...
    bucket : str
        GCS bucket name (no scheme). Tables saved under disalexi_tables/<mask_type>/.
    ...
    for fid, row in tqdm(df.iterrows(), desc='Export DisALEXI zonal stats', total=df.shape[0]):
        ...
        for year in range(start_yr, end_yr + 1):
            desc = f'disalexi_etf_{fid}_{mask_type}_{year}'
            fn_prefix = os.path.join('disalexi_tables', mask_type, desc)

            if check_dir:
                f = os.path.join(check_dir, mask_type, f'{desc}.csv')
                if os.path.exists(f):
            ...
            for img_id in scenes:
                ...
                try:
                    # Create DisALEXI image with reference ET parameters
                    # Note: DisALEXI uses from_landsat_c02_l2 (not c2_sr)
                    disalexi_img = disalexi.Image.from_landsat_c02_l2(
                        img_id,
                    ...
                    # DisALEXI doesn't have et_fraction property, compute manually
                    et_img = disalexi_img.et
                    et_ref_img = disalexi_img.et_reference
                    etf_img = et_img.divide(et_ref_img).rename(_name)
...
if __name__ == '__main__':
    ...
    chk_dir = os.path.join(data, 'landsat', 'extracts', 'disalexi_etf')
