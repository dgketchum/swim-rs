"""
SIMS ET fraction zonal statistics export module.

Export per-scene SIMS ET fraction zonal means for polygons to Google Cloud Storage as CSVs.
"""
import os
...
def export_sims_zonal_stats(
    ...
    bucket : str
        GCS bucket name (no scheme). Tables saved under sims_tables/<mask_type>/.
    ...
    for fid, row in tqdm(df.iterrows(), desc='Export SIMS zonal stats', total=df.shape[0]):
        ...
        for year in range(start_yr, end_yr + 1):
            ...
            fn_prefix = os.path.join('sims_tables', mask_type, desc)
            ...
            for img_id in scenes:
                ...
                try:
                    # Create SIMS image and get ET fraction
                    sims_img = sims.Image.from_landsat_c2_sr(img_id)
