"""
SSEBop ET fraction zonal statistics export module.
...
def export_ssebop_zonal_stats(
    ...
    bucket : str
        GCS bucket name (no scheme). Tables saved under ssebop_tables/<mask_type>/.
    ...
    for fid, row in tqdm(df.iterrows(), desc='Export SSEBop zonal stats', total=df.shape[0]):
        ...
        for year in range(start_yr, end_yr + 1):
            ...
            fn_prefix = os.path.join('ssebop_tables', mask_type, desc)
            ...
            for img_id in scenes:
                ...
                # Create SSEBop image and get ET fraction
                ssebop_img = ssebop.Image.from_landsat_c2_sr(img_id)
