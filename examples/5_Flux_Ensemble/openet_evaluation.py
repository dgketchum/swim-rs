...
def evaluate_openet_site(
    ...
    flux_data_path: str,
    openet_daily_path: str | None,
    openet_monthly_path: str | None,
    irr: dict[str, Any],
    gap_tolerance: int = 5,
...
def _load_openet_daily(
    df: pd.DataFrame, openet_daily_path: str | None, openet_models: list[str]
...
def _load_openet_monthly(
    openet_monthly_path: str | None, openet_models: list[str]
) -> pd.DataFrame | None:
    """Load OpenET monthly data."""
...
def _compute_metrics(
    df: pd.DataFrame, flux_col: str, model_cols: list[str], min_samples: int = 1
) -> dict[str, float]:
...
def _compute_monthly_metrics(
    ...
    all_models: list[str],
    gap_tolerance: int,
    openet_monthly: pd.DataFrame | None,
