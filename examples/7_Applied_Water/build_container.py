"""Build an Example 7 calibration container from the getInfo base container.
...
``7_Applied_Water_{run}.swim`` rather than E5's hard-coded ``5_Flux_Ensemble_``
prefix. Steps are identical to E5: copy the base container, re-ingest the 6-model
OpenET v2.1 ETf (ETo denominator, MAX dedup, 2016-2025), ingest OpenET reference
ETo/ETr, compute the simple mean ensemble, recompute dynamics, validate.

ETf/refET CSVs are read from this project's repo data dir
(``examples/7_Applied_Water/data/{etf_v21_openet_eto,openet_refet}/``) — the same
...
data volume; ETf/refET CSVs live beside the example scripts).
    ...
    uv run python examples/7_Applied_Water/build_container.py --run e7cal
"""
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Build Example 7 calibration container")
    parser.add_argument("--run", required=True, help="Run name (e.g. e7cal)")
    parser.add_argument(
