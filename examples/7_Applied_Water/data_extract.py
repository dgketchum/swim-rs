...
identical to E2 — these fields are CONUS, so the same inputs apply. E2's driver
lives inline under its own ``__main__`` (no callable ``main()``), so we replicate
the step dispatch here against E2's public ``extract_*`` functions.
...
# ``Path(__file__).parent`` (the E2 module's location). Repoint E2's module
# ``__file__`` at this project so those CSVs land in examples/7_Applied_Water/
...
# it. E7 uses runoff_process="cn" (NLDAS not needed), so strip the stale kwarg.
