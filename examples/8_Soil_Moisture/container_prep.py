"""Container prep for Example 8 (SCAN soil moisture), reusing Example 5.

Thin wrapper over Example 5 ``container_prep``: repoints the config loader at
``8_Soil_Moisture.toml`` and replaces ``build_shapefile`` with a guard, because
our fields shapefile is prebuilt by ``build_scan_fields.py`` (it is not derived
from flux-tower footprints). Everything else — GridMET mapping, container
creation, ingest of met/RS/snow/properties, fused NDVI, dynamics — is identical
to Example 5.

    uv run python examples/8_Soil_Moisture/container_prep.py --overwrite --getinfo
"""
...
def _guard_shapefile(cfg, overwrite=False, exclude_sites=None):
    """The shapefile is prebuilt by build_scan_fields.py; do not rebuild from flux data."""
    if not os.path.exists(cfg.fields_shapefile):
        raise SystemExit(
            f"Fields shapefile not found: {cfg.fields_shapefile}\n"
            "Run examples/8_Soil_Moisture/build_scan_fields.py first."
        )
...
def main() -> None:
    p = argparse.ArgumentParser(description="Container prep for 8_Soil_Moisture")
