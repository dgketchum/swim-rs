...
def initialize_master_dict(shape):
    ...
    master = {'tot_{}'.format(key): np.zeros(shape) for key in keys}

    master['pkcb'] = None
    master['albedo'] = np.ones(shape) * 0.45
    master['swe'] = np.zeros(shape)
    master['dry_days'] = np.zeros(shape)
...
def initialize_static_dict(config):
    ...
    taw = d['taw']
    tew = d['tew']
    print('original Ksat = {}, REW = {}, TAW = {}'.format(d['soil_ksat'], d['rew'], d['taw']))

    # apply high TAW to unconsolidated Quaternary deposits
    min_val = 250
    taw = np.maximum(min_val, taw)
...
def initialize_initial_conditions_dict(config):
    """
