"""Shared machinery for the revision-5 Figure 1 panel-(a) design studies.

Revision 5 is a **Level 0/1 presentation reset** (handoff Sec. 16) against the
frozen revision-4 data. Nothing here re-extracts, rebuilds, or re-contracts
anything: the same `paper/data/final/figures/` package is read, and the studies
...
# ===========================================================================
# panel (b) -- FIXED this round (handoff Sec. 16.3) except the E3 key
# ===========================================================================
...
E3_FRAME = (152.0, 13.6, 33.0, MAP_TOP - 13.6)
E3_KEY_X = 152.0
E3_KEY_LINES = (10.0, 6.8, 3.6)  # baselines, top to bottom
...
def audit_scientific(F: Frozen, axis_audit: dict, obs_ink: list, cycle: dict) -> dict:
    ...
    for rid, a in axis_audit.items():
        ...
        # Sec. 5.2: "a trace that clears the upper limit by only a line width is
        # not visually adequate even if it passes a numeric no-clip test"
        frac = (d1 - hi) / span
...
def export(fig, stem: str, extra: dict) -> dict:
