...
class SamplePlotDynamics:
    def __init__(self, landsat_file, irr_csv_file, out_json_file, irr_threshold=0.1, select=None):
    ...
    def analyze_irrigation(self):
        ...
        for field in tqdm(self.irr.index, desc='Analyzing Irrigation', total=len(self.irr.index)):
            if self.select and field not in self.select:
                continue

            field_data = self._analyze_field_irrigation(field)
    ...
    def _load_data(self):
        self.lst = pd.read_csv(self.landsat_file, index_col=0, parse_dates=True)
        self.years = list(set([i.year for i in self.lst.index]))
    ...
    def _analyze_field_irrigation(self, field):
        ...
        for yr in self.years:
            ...
            df = self.lst.loc[f'{yr}-01-01': f'{yr}-12-31', [selector]]
...
if __name__ == '__main__':
    ...
    cuttings_json = os.path.join(landsat, 'calibration_cuttings_.json')

    dynamics = SamplePlotDynamics(remote_sensing_file, irr, irr_threshold=0.1,
                                  out_json_file=cuttings_json, select='S2')
