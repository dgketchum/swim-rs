...
def compare_etf_estimates(
    combined_output_path,
    flux_data_path,
    openet_daily_path=None,
    openet_monthly_path=None,
    irr=None,
    target_model="ssebop",
    gap_tolerance=5,
    ssebop_eto_source="eto_corr",
    ...
    output.loc[irr_index, "capture"] = ~np.isnan(
        output.loc[irr_index, f"{target_model}_etf_irr"].values
    )
    ...
    if openet_monthly_path:
        try:
            ...
            idx = pd.to_datetime(
                [d.replace(day=pd.Timestamp(d).days_in_month) for d in openet_monthly.index]
            )
    ...
    for model in all_models:
        ...
        if df_temp.empty:
            continue
        try:
            results_daily[f"rmse_{model}"] = float(
                np.sqrt(mean_squared_error(df_temp["flux"], df_temp[model]))
            )
    ...
    for model in all_models:
        ...
        if df_temp.shape[0] >= 2:
            try:
                results_overpass[f"rmse_{model}"] = float(
                    np.sqrt(mean_squared_error(df_temp["flux"], df_temp[model]))
                )
    ...
    # Removed legacy local ETD debug injection
