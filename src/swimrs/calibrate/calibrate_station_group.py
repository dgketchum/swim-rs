...
def run_pest_sequence(
    conf_path, project_ws, workers, realizations, bad_params=None, pdc_remove=False
):
    """"""
    ...
    prep_fields_json(
        properties_json, joined_timeseries, dynamics_data, prepped_input, target_plots=fids
    )
    ...
    p_dir = os.path.join(project_ws, "tight_pest")
    if os.path.isdir(p_dir):
        shutil.rmtree(p_dir)

    m_dir = os.path.join(project_ws, "tight_master")
    ...
    spinup_out = os.path.join(target_dir, "spinup.json")
...
if __name__ == "__main__":
    ...
    if not os.path.isdir(d):
        ...
        d = os.path.join(home, "code", "swim-rs", "examples")
    ...
    bad_parameters = (
        "/home/dgketchum/code/swim-rs/examples/4_Flux_Network/results_comparison_bad.csv"
    )
