...
class ExportStep(WorkflowStep):
    """Step for exporting prepared inputs."""
    ...
    @property
    def name(self) -> str:
        return "export_prepped_input"

    @property
    def description(self) -> str:
        return f"Export prepared inputs to {self.output_path}"
    ...
    def execute(self, container: "SwimContainer") -> StepResult:
        ...
        try:
            ...
            return StepResult(
                status=StepStatus.COMPLETED,
                message=f"Exported to {self.output_path}",
                duration_seconds=duration,
            )
        except Exception as e:
            return StepResult(
