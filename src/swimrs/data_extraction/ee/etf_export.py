...
def export_etf_images(
    ...
    coll = ee.ImageCollection(ETF).filterDate(f"{year}-01-01", f"{year}-12-31")
...
def sparse_sample_etf(
    ...
    for fid, row in tqdm(df.iterrows(), desc="Processing Fields", total=df.shape[0]):
        for year in range(start_yr, end_yr + 1):
            ...
            desc = f"{model}_etf_{site}_{mask_type}_{year}"
            ...
            etf_coll = ee.ImageCollection(source).filterDate(f"{year}-01-01", f"{year}-12-31")
            ...
            for img_id in etf_scenes:
                ...
                if source is not None and scale is not None:
                    ...
                    refet = ee.Image(
                        f"projects/openet/assets/reference_et/conus/gridmet/daily/v1/{_dt}"
                    ).select("eto")
...
def clustered_sample_etf(
    ...
    if not (all_west or all_east):
        raise ValueError(
            "clustered_sample_etf requires all features be either in WEST_STATES or all outside; mixed states detected"
        )
    use_west = all_west
    ...
    for year in range(start_yr, end_yr + 1):
        ...
        coll = ee.ImageCollection(_source).filterDate(f"{year}-01-01", f"{year}-12-31")
        coll = coll.filterBounds(feature_coll)
        scenes = coll.aggregate_histogram("system:index").getInfo()
        ...
        for img_id in scenes:
            ...
            if mask_type == "no_mask":
                ...
                etf_masked = etf_img.clip(feature_coll.geometry()).mask(
                    irr_mask_west if use_west else irr_mask_east
                )
            elif mask_type == "inv_irr":
                etf_masked = etf_img.clip(feature_coll.geometry()).mask(
                    irr.gt(0) if use_west else irr_east.gt(0)
                )
            else:
                etf_masked = etf_img.clip(feature_coll.geometry())
...
def export_to_cloud(
    ...
    if dest == "bucket":
        ...
        try:
            existing_geotiffs = (
                subprocess.check_output(["gsutil", "ls", f"gs://{bucket}/{file_prefix}/geotiff/"])
                .decode("utf-8")
                .split("\n")
            )
            existing_geotiffs = [os.path.basename(f) for f in existing_geotiffs if f]
            existing_metadata = (
                subprocess.check_output(["gsutil", "ls", f"gs://{bucket}/{file_prefix}/metadata/"])
                .decode("utf-8")
                .split("\n")
            )
