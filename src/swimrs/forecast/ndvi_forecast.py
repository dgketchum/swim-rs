...
def forecast_ndvi(
    ...
    for year in target_years:
        ...
        for compare_year in sample_years:
            ...
            end = pd.to_datetime(f"{compare_year}-01-01") + pd.Timedelta(
                days=last_obs_doy + similarity_window // 2 - 1
            )
        ...
        current_data = df.loc[
            last_obs_date - pd.Timedelta(days=similarity_window // 2) : last_obs_date
            + pd.Timedelta(days=similarity_window // 2),
            [current_ndvi_col, ct_col],
        ]
        ...
        similarity_scores = {
            yr: np.sum((current_data.values - hist_data.values) ** 2)
            for yr, hist_data in similar_years_data.items()
        }

        most_similar_years = sorted(similarity_scores, key=similarity_scores.get)[
            :max_years_to_consider
        ]

        for compare_year in most_similar_years:
            for day in range(1, forecast_days + 1):
                f_date = pd.to_datetime(f"{compare_year}-01-01") + pd.Timedelta(
                    days=last_obs_doy + day - 1
                )
                try:
                    f_irr = plots.input["irr_data"][field_id][str(compare_year)]["f_irr"]
    ...
    results["historical_data"] = {
        k.strftime("%Y-%m-%d"): v for k, v in results["historical_data"].items()
    }
...
def plot_ndvi_forecast(results, output_dir, field_id):
    ...
    hist_values_filtered = [
        hist_values[i] for i, d in enumerate(hist_dates) if d >= plot_start_date
    ]

    fig.add_trace(
        go.Scatter(
            x=hist_dates_filtered, y=hist_values_filtered, mode="lines", name="Historical Data"
        )
    )

    for day in range(1, len(results["forecasts"]) + 1):
        x_coords = [last_obs_date + pd.Timedelta(days=int(day))] * len(
            results["forecasts"][str(day)]
        )
        if results["forecasts"][str(day)]:
            fig.add_trace(
                go.Scatter(
                    x=x_coords,
                    y=results["forecasts"][str(day)],
                    mode="markers",
                    marker={
                        "size": 5,
                        "opacity": 0.7,
...
if __name__ == "__main__":
    ...
    run_data = os.path.join(root, "examples")
    ...
    for site_ in sites:
        ...
        prepped_input = os.path.join(output_, "prepped_input.json")
        ndvi_forecast_ = os.path.join(output_, "ndvi_forecast.json")
        ...
        forecast_ndvi(
            ...
            site_,
            forecast_days=150,
            output_json=ndvi_forecast_,
            similarity_window=7,
            max_years_to_consider=10,
        )

        out_fig_dir_ = os.path.join(root, "examples", project, "figures", "ndvi_forecast", "html")
