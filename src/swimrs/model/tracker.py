...
class SampleTracker:
    ...
    def __init__(self, config, plots, size):
        ...
        self.aw = 0
        self.cum_evap = 0.0
        self.cum_evap_prev = 0.0
        self.depl_ze = 0.0
        self.daw3 = 0.0
        self.daw3_prev = 0.0
        self.depl_root = 0.0
        self.depl_root_prev = 0.0
        self.soil_water = 0.0
        self.soil_water_prev = 0.0
        self.dperc = 0.0
    ...
    def load_root_depth(self):
        ...
        self.zr = np.array(
            [rz if cd in perennials else self.zr_min[0, 0] for rz, cd in zip(rz_depth, codes)]
        ).reshape(1, -1)

        self.zr_max = np.array([rz for rz in rz_depth]).reshape(1, -1)
        self.zr_min = np.array(
            [rz if cd in perennials else self.zr_min[0, 0] for rz, cd in zip(rz_depth, codes)]
        ).reshape(1, -1)
    ...
    def load_soils(self):
        ...
        if self.conf.calibrate or self.conf.forecast:
            ...
            self.aw = (
                np.array([self.plots.input["props"][f]["awc"] for f in fields]).reshape(1, -1)
                * 1000.0
            )

        self.ksat = np.array([self.plots.input["props"][f]["ksat"] for f in fields]).reshape(1, -1)
        self.ksat = self.ksat * 0.001 * 86400.0
        self.ksat_hourly = np.ones((24, self.ksat.shape[1])) * self.ksat / 24.0
        ...
        clay = np.array([self.plots.input["props"][f].get("clay", 20.0) for f in fields]).reshape(
            1, -1
        )
        ...
        if self.conf.calibrate:
            ...
            for k, f in self.conf.calibration_files.items():
                ...
                while not param_found:
                    ...
                    if ct > 1000:
                        raise ValueError(
                            f"Calibration Parameter Match Not Found\n Parameter {k} not in {f}"
                        )
            ...
            if skipped_fields:
                ...
                warnings.warn(
                    ...
                    f"{skipped_list[:10]}"
                    + (f"... and {len(skipped_list) - 10} more" if len(skipped_list) > 10 else "")
                )
            ...
            for k, v in self.conf.forecast_parameters.items():
                ...
                while not param_found:
                    ...
                    if ct > 1000:
                        raise ValueError(
                            f"Forecast Parameter Match Not Found\n Parameter {k} not in {v}"
                        )
    ...
    def apply_initial_conditions(self):
        ...
        if self.conf.spinup and os.path.exists(self.conf.spinup):
            with open(self.conf.spinup) as fp:
                sdct = json.load(fp)
