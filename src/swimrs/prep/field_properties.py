...
def write_field_properties(
    ...
    **kwargs,
        ...
        for k, v in elc.items():
            if k in dct:
                ...
                if val == 10 and dct[k]["lulc_code"] != 12:
                    print(
                        f"{k}: Override MODIS non-crop code {dct[k]['lulc_code']} with crop code from {elc_key}"
                    )
                    dct[k] = {
        ...
        for k, v in irr_dct.items():
            if k in dct:
                ...
                if irr_val > 0.3 and dct[k]["lulc_code"] != 12:
                    print(
                        f"{k}: Override MODIS non-crop code {dct[k]['lulc_code']} with crop code from irrigation mask"
                    )
                    dct[k] = {
                        "root_depth": RZ.get(str(12), {}).get("rooting_depth", np.nan),
                        "zr_mult": RZ.get(str(12), {}).get("zr_multiplier", np.nan),
    ...
    for k, v in dct.items():
        ...
        elif "area_sq_m" in v and v["area_sq_m"] < 900.0:
            _ = d.pop(k)
