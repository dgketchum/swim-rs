...
def join_daily_timeseries(
    ...
    **kwargs,
    ...
    if snow is not None:
        with open(snow) as fp:
            snow_dct = json.load(fp)

    for fid, row in tqdm(
        field_df.iterrows(), total=field_df.shape[0], desc="Processing Time Series files"
    ):
        ...
        out_file = os.path.join(dst_dir, f"{fid}.parquet")
        if os.path.exists(out_file) and not overwrite and time_covered:
            continue

        if met_map_key is not None:
            target = row[met_map_key]
            met_file = os.path.join(met_dir, f"{target}.parquet")
        else:
            met_file = os.path.join(met_dir, f"{fid}.parquet")
