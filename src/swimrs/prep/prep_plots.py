...
def prep_fields_json(
    properties, time_series, dynamics, out_js, rs_params, target_plots=None, interp_params=None
):
    """
    ...
    missing += [
        x for x in target_plots if not os.path.exists(os.path.join(time_series, f"{x}.parquet"))
    ]
    missing = list(set(missing))

    if missing:
        print(f"Target sample(s) missing from time series data: {missing}")
    ...
    for fid, v in tqdm(
        dct["props"].items(), total=len(dct["props"]), desc="Writing model input json"
    ):
        if fid in missing:
            continue

        _file = os.path.join(time_series, f"{fid}.parquet")
        ...
        if first:
            ...
            if not df.shape[0] == shape:
                print(f"{fid} does not have shape {df.shape[0]}")
                continue
            ...
            try:
                if interp_params is not None and param in interp_params:
                    ...
                    if int_arr.shape[1] > 1:
                        ...
                        sent_adj_ndvi = sentinel_adjust_quantile_mapping(
                            sentinel_ndvi_df=sndvi_unadj,
                            landsat_ndvi_df=lndvi,
                            min_pairs=20,
                            window_days=1,
                        )
                        int_arr = pd.concat(
                            [lndvi, pd.DataFrame(sent_adj_ndvi)], ignore_index=False, axis=1
                        ).mean(axis=1)
    ...
    for fid in fields.input["order"]:
        ...
        if col_inv in data.columns:
            ...
            try:
                irr_years = [
                    int(k)
                    for k, v in fields.input["irr_data"][fid].items()
        ...
        _file = os.path.join(config.obs_folder, f"obs_etf_{fid}.np")
        np.savetxt(_file, data["etf"].values)

        print("preproc SWE mean: {:.2f}\n".format(np.nanmean(data["swe"].values)))
        _file = os.path.join(config.obs_folder, f"obs_swe_{fid}.np")
        np.savetxt(_file, data["swe"].values)

        ct += 1

    print(f"Prepped {ct} fields input")
