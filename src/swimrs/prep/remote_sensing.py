...
def sparse_time_series(
    ...
    file_list = [os.path.join(csv_dir, x) for x in os.listdir(csv_dir) if x.endswith(".csv")]
    if len(file_list) == 0:
    ...
    target_columns = pd.MultiIndex.from_tuples(target_columns, names=COLUMN_MULTIINDEX)
    ...
    if len(empty_yrs) > 0:
        ...
        print(
            f"{len(empty_yrs)} years without remote sensing data, {empty_yrs[0]} to {empty_yrs[-1]}"
        )
        dt_index = pd.date_range(f"{empty_yrs[0]}-01-01", f"{empty_yrs[-1]}-12-31", freq="D")
        adf = pd.DataFrame(
            data=np.zeros((len(dt_index), len(gdf.index))) * np.nan,
            index=dt_index,
    ...
    for yr in tqdm(rs_years, total=len(rs_years), desc=f"Processing data from {source[-2]}"):
        dt_index = pd.date_range(f"{yr}-01-01", f"{yr}-12-31", freq="D")
        ...
        if len(yr_files) > 0:
            for f in yr_files:
                ...
                if len(f_idx) != len(f_idx_dt):
                    ...
                    out_datestrs = [
                        datetime.strftime(i, "%Y%m%d") for i in f_idx_dt if i not in f_idx
                    ]
    ...
    try:
        ...
        print(
            f"Model '{algorithm}', mask '{mask}': {nonzero_counts}/{len(stations_with_data)} stations with any observations."
        )
    except Exception:
        pass
        ...
        for sid, alg_dict in obs_counts.items():
            ...
            for alg, mask_dict in alg_dict.items():
                ...
                for msk, year_dict in mask_dict.items():
                    serializable[str(sid)][str(alg)][str(msk)] = {
                        str(int(y)): int(c) for y, c in year_dict.items()
                    }
...
def join_remote_sensing(files, dst, station_selection="exclusive"):
    ...
    valid_indices = [
        df.index for df in dfs if isinstance(df.index, pd.DatetimeIndex) and not df.index.empty
    ]
    ...
    for sid in common_sids:
        ...
        site_df = pd.DataFrame(
            data=np.array(target_series).T, index=new_daily_index, columns=target_cols
        )
