...
def join_site_timeseries(irr_dir, unirr_dir, joined_dir, summary_json, missing_json):
    ...
    missing = {"irr_missing": [], "uirr_missing": [], "irr_partial": [], "uirr_partial": []}
    ...
    for i, sid in enumerate(ids):
        ...
        try:
            ...
            idf.columns = [f"i_{c}" for c in idf.columns]
        ...
        try:
            ...
            udf.columns = [f"u_{c}" for c in udf.columns]
        ...
        try:
            sum_ = (
                joined_df.loc[
                    "2012-01-01":"2023-12-31", ["i_irrigation", "i_et_act", "u_et_act"]
                ].sum(axis=0)
                / 10
            )
            ...
            print(
                f"{str(i).rjust(4, '.')}: {sid.rjust(6, '.')}; irr: {irr:5d}, i_eta: {i_eta:5d},"
