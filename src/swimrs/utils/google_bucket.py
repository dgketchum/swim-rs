...
def list_and_copy_gcs_bucket(
    cmd_path, bucket_path, local_dir, glob="*", dry_run=False, overwrite=False
):
    """"""
    prepend = "gs://"
    ...
    for file_path in tqdm(
        files_to_copy, desc=f"Copying Files from {bucket_path}", total=len(files_to_copy)
    ):
        if file_path:
...
if __name__ == "__main__":
    ...
    for mask in ["inv_irr", "irr"]:
        dst = os.path.join(root, f"examples/tutorial/landsat/extracts/etf/{mask}")
        glob_ = f"etf_{mask}"
