                    ...
                    if f.endswith(".json"):
                        ...
                        try:
                            with open(fp) as h:
                                ...
                                instrument_hint = (
                                    "sentinel"
                                    if "sentinel" in fp.lower()
                                    else ("landsat" if "landsat" in fp.lower() else None)
                                )
                                ...
                                try:
                                    ...
                                    level2 = (
                                        next(iter((level1 or {}).values()))
                                        if isinstance(level1, dict)
                                        else None
                                    )
                                    inner = (
                                        next(iter((level2 or {}).values()))
                                    ...
                                    if instrument_hint is not None:
                                        ...
                                        for sid, models in (loaded or {}).items():
                                            if isinstance(models, dict) and "none" in models:
                                                ...
                                                models_copy[f"none_{instrument_hint}"] = (
                                                    models_copy.pop("none")
                                                )
            ...
            try:
                with open(p) as h:
                    ...
                    instrument_hint = (
                        ...
                        else ("landsat" if "landsat" in p.lower() else None)
                    )
                    # Preserve NDVI instrument by wrapping mask-level under 'none_<instrument>'
                    # or by renaming model 'none' -> 'none_<instrument>' for model-level inputs
                    try:
...
def _merge_counts(
    dicts: list[dict[str, dict[str, dict[str, int | dict[str, int]]]]],
...
def summarize_observation_counts(
    ...
    mask_whitelist: list[str] | None = None,
    station_whitelist: list[str] | None = None,
    select_stations: list[str] | None = None,
    ...
    for sid, models in merged.items():
        ...
        for model in models_to_use:
            ...
            for msk in masks_to_use:
                year_counts = {
                    int(y): int(c)
                    for y, c in (masks.get(msk, {}) or {}).items()
                    if isinstance(y, int)
                }
                ...
                station_mask_presence_overall[msk] = (
                    station_mask_presence_overall.get(msk, False) or has_any_data
                )
                ...
                rows.append(
                    {
                        ...
                        "years_with_data": ",".join(map(str, years_with_data))
                        if years_with_data
                        else "",
    ...
    try:
        for sid, models in merged.items():
            ...
            if missing_years:
                ...
                print(
                    f"[rs_diagnostics] Station {sid} lacks irr/inv_irr observations in years: {years_str}"
                )
    except Exception:
    ...
    instrument_summary: dict[str, dict[str, dict[str, dict[str, float | list[int]]]]] = {}
    for sid, models in merged.items():
