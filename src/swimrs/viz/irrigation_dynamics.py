...
from swimrs.prep import get_ensemble_parameters, get_flux_sites, prep_fields_json
...
def irrigation_timeseries(field_data, feature, out_dir=None):
    for year in range(1987, 2023):
        ...
        column, desc, color = "ndvi_irr", f"Irrigated NDVI (Smoothed) - {feature}", "green"
        ...
        fig.add_trace(
            go.Scatter(
                x=df_year.index,
                y=df_year[column + "_rolling"],
                mode="lines",
                name=desc + " 32-day Mean",
...
if __name__ == "__main__":
    ...
    for ee, site_ in enumerate(sites):
        ...
        prep_fields_json(
            ...
            config.plot_timeseries,
            dynamics,
            station_prepped_input,
            target_plots=[site_],
            rs_params=rs_params_,
            interp_params=("ndvi",),
        )
