...
def plot_swim_timeseries(
        ...
        if param in bar_vars:
            ...
            if param == "flux_etf":
                ...
                secondary_y = (
                    False
                    if (
                        param in ["et_act", "etref"]
                        and "et_act" in parameters
                        and "etref" in parameters
                    )
                    else True
                    if i > 0
                    else False
    ...
    for param in parameters:
        if param in ["etf", "etf_irr", "etf_inv_irr", "ndvi", "ndvi_irr", "ndvi_inv_irr"]:
            ...
            if ct_param in df.columns:
                ...
                if "pdc" in df.columns and (
                    param == "etf_irr" or param == "etf_inv_irr" or param == "etf"
                ):
                    ...
                    trace = go.Scatter(
                        x=scatter_df_pdc.index,
                        y=scatter_df_pdc[param],
                        mode="markers",
                        marker_symbol="circle",
                        marker_size=15,
                        name=f"{param.split('_')[0]} retrieval (PDC Flagged)",
                        marker=dict(color=COLOR_MAP.get(param, "black")),
                    )
                    _add_trace(param + "_pdc_markers", trace, main_row, True)

                    trace = go.Scatter(
                        x=scatter_df_no_pdc.index,
                        y=scatter_df_no_pdc[param],
                        mode="markers",
                        marker_symbol="x",
                        marker_size=5,
                        name=f"{param.split('_')[0]} retrieval",
                        marker=dict(color=COLOR_MAP.get(param, "black")),
                    )
    ...
    if members and irr_index is not None:
        for member in members:
            ...
            if member_etf_irr_col in df.columns and member_etf_inv_irr_col in df.columns:
                ...
                plotly_rgba_color = (
                    f"rgba({int(rgba_tuple[0] * 255)}, {int(rgba_tuple[1] * 255)}, "
    ...
    kwargs = dict(
        title_text="SWIM Model Time Series",
        xaxis_title="Date",
        yaxis_title="mm",
...
def flux_pdc_timeseries(
    ...
    for fid in fids:
        ...
        if os.path.exists(flux_file):
            ...
            flux_yr = list(
                set(
                    [i.year for i, obs in flux_obs.iterrows() if pd.notna(obs.get("EToF_filtered"))]
                )
            )
