...
def test_cli_extract_single_site_smoke(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        cli, "sample_snodas_swe", lambda **kwargs: calls.__setitem__("snodas", calls["snodas"] + 1)
    )
    monkeypatch.setattr(cli, "get_cdl", lambda *a, **k: calls.__setitem__("cdl", calls["cdl"] + 1))
    monkeypatch.setattr(
        cli, "get_irrigation", lambda *a, **k: calls.__setitem__("irr", calls["irr"] + 1)
    )
    monkeypatch.setattr(
        cli, "get_ssurgo", lambda *a, **k: calls.__setitem__("ssurgo", calls["ssurgo"] + 1)
    )
    monkeypatch.setattr(
        cli, "get_landcover", lambda *a, **k: calls.__setitem__("landcover", calls["landcover"] + 1)
    )
    monkeypatch.setattr(
        cli, "sparse_sample_ndvi", lambda *a, **k: calls.__setitem__("ndvi", calls["ndvi"] + 1)
    )
    monkeypatch.setattr(
        cli, "sparse_sample_etf", lambda *a, **k: calls.__setitem__("etf", calls["etf"] + 1)
    )
    monkeypatch.setattr(
        cli,
        "assign_gridmet_and_corrections",
        lambda *a, **k: calls.__setitem__("gm_map", calls["gm_map"] + 1),
    )
    monkeypatch.setattr(
        cli, "download_gridmet", lambda *a, **k: calls.__setitem__("gm_dl", calls["gm_dl"] + 1)
    )
...
def test_cli_prep_single_site_smoke(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        cli,
        "sparse_time_series",
        lambda *a, **k: calls.__setitem__("sparse_ts", calls["sparse_ts"] + 1),
    )
    monkeypatch.setattr(
        cli,
        "join_remote_sensing",
        lambda *a, **k: calls.__setitem__("join_rs", calls["join_rs"] + 1),
    )
    monkeypatch.setattr(
        cli,
        "write_field_properties",
        lambda *a, **k: calls.__setitem__("write_props", calls["write_props"] + 1),
    )
    monkeypatch.setattr(
        cli,
        "create_timeseries_json",
        lambda *a, **k: calls.__setitem__("snodas_js", calls["snodas_js"] + 1),
    )
    monkeypatch.setattr(
        cli,
        "join_daily_timeseries",
        lambda *a, **k: calls.__setitem__("join_daily", calls["join_daily"] + 1),
    )
    monkeypatch.setattr(
        cli, "process_dynamics_batch", lambda *a, **k: calls.__setitem__("dyn", calls["dyn"] + 1)
    )
    monkeypatch.setattr(
        cli, "prep_fields_json", lambda *a, **k: calls.__setitem__("prep_js", calls["prep_js"] + 1)
    )
