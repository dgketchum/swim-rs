...
class TestEtfWeightAssignment:
    ...
    def test_etf_weight_uses_default_when_etf_std_fid_is_none(self):
        ...
        assert (capture_weights > 0).all(), \
            "All capture dates should have positive weights"
    ...
    def test_etf_weight_uses_std_when_etf_std_fid_has_data(self):
        ...
        # i.e., ~4 to ~6.67
        assert capture_weights.min() > 3.5, \
            f"Min weight {capture_weights.min()} too low"
        assert capture_weights.max() < 7.0, \
            f"Max weight {capture_weights.max()} too high"
    ...
    def test_buggy_condition_would_fail(self):
        ...
        # NEW fixed logic
        new_logic_result = (
            etf_std_with_none is not None and
            etf_std_with_none.get(fid) is not None
        )
